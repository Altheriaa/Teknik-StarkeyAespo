...
export default function PengurusanSurat({ PengurusanSurats, filters }) {
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(() => {
            router.get('/pengurusan-surat', { search: searchTerm }, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
        ...
        <HomeLayout header="Kegiatan Akademik">
            <section className="hero">
                <div className="container-hero text-center">
                    <h1 className="display-4 fw-bold fade-in">Pengurusan Surat</h1>
                        ...
                        <table className="download-table table table-hover" id="sop-download-table">
                            <thead className="download-table-header table-dark">
                                <tr>
                                    ...
                                    <th className="download-col-name">PENGURUSAN SURAT</th>
                                    <th className="download-col-date">TANGGAL</th>
                                    <th className="download-col-action">AKSI</th>
                                </tr>
                            </thead>
                            <tbody className="download-table-body">
                                {PengurusanSurats.data.map((PengurusanSurat, index) => (
                                    <tr key={index}>
                                        <td className="text-center">
                                            {PengurusanSurats.from + index}
                                        </td>
                                        <td>{PengurusanSurat.pengurusan_surat}</td>
                                        <td>{PengurusanSurat.date}</td>
                                        <td className="text-center"><a href={PengurusanSurat.link} target='_blank'
                    ...
                    <div className="d-flex justify-content-center mt-4">
                        <nav className="mt-4">
                            <ul className="pagination justify-content-center">
                                {PengurusanSurats.links.map((link, i) => (
                                    <li
                                        key={i}
                                        className={`page-item 
                                            ${link.active ? 'active' : ''} 
