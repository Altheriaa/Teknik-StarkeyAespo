...
export default function DokumenAmi ({ DokumenAmis, filters }) {
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(() => {
            router.get('/dokumen-ami', { search: searchTerm }, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
    ...
    return (
        <HomeLayout header="Dokumen AMI">
            <section className="hero">
                <div className="container-hero text-center">
                    <h1 className="display-4 fw-bold fade-in">Dokumen AMI</h1>
                    ...
                    <div className="download-table-wrapper table-responsive">
                        <table className="download-table table table-hover" id="sop-download-table">
                            ...
                            <tbody className="download-table-body">
                                {DokumenAmis.data.map((DokumenAmi, index) => (
                                    <tr key={index}>
                                        <td className="text-center">
                                            {DokumenAmis.from + index}
                                        </td>
                                        <td>{DokumenAmi.nama_dokumen}</td>
                                        <td className='text-center'>{DokumenAmi.date}</td>
                                        <td className="text-center"><a href={DokumenAmi.link} target='_blank'
