...
export default function DokumenMonev ({ DokumenMonevs, filters }) {
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(() => {
            router.get('/dokumen-monev', { search: searchTerm }, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
    ...
    return (
        <HomeLayout header="Dokumen Monev">
            <section className="hero">
                <div className="container-hero text-center">
                    <h1 className="display-4 fw-bold fade-in">Dokumen Monev</h1>
                        ...
                        <table className="download-table table table-hover" id="sop-download-table">
                            <thead className="download-table-header table-dark">
                                <tr>
                                    ...
                                    <th className="download-col-name">NAMA DOKUMEN</th>
                            ...
                            <tbody className="download-table-body">
                                {DokumenMonevs.data.map((DokumenMonev, index) => (
                                    <tr key={index}>
                                        <td className="text-center">
                                            {DokumenMonevs.from + index}
                                        </td>
                                        <td>{DokumenMonev.nama_dokumen}</td>
                                        <td className='text-center'>{DokumenMonev.date}</td>
                                        <td className="text-center"><a href={DokumenMonev.link} target='_blank'
                    ...
                    <div className="d-flex justify-content-center mt-4">
                        <nav className="mt-4">
                            <ul className="pagination justify-content-center">
                                {DokumenMonevs.links.map((link, i) => (
                                    <li
                                        key={i}
                                        className={`page-item 
                                            ${link.active ? 'active' : ''} 
